...
# 筛选 6， 7， 8
cnn_res = cnn_res[np.isin(cnn_res.index.month, [6, 7, 8])]
ds = ds.sel(time=ds.time[np.isin(ds.time.dt.month, [6, 7, 8])])
...
axes[4].format(title='Quantitative partitioning (JJA)', titleloc='left', abcloc='l', 
               ylim=(-5, 80), yticks=[0, 20, 40, 60, 80], 
...
print(f'''
      极端降水变化: {delta_dt.values[0, 0]:.2f} mm/JJA
...
plt.savefig('./pics/FIG9_分解.eps', dpi=400)
plt.savefig('./pics/FIG9_分解.png', dpi=400)
plt.show()
...
units_abs = ['mm/JJA', 'mm/JJA']
units_rla = ['%', '%']
cmaps = ['anomaly', 'anomaly']

titles = ['Therm EPCP JJA', 'Dynam EPCP JJA']

levels_rla = [np.linspace(-125, 125, 11),
              np.linspace(-125, 125, 11)]

levels_abs = [np.linspace(-70, 70, 11),
              np.linspace(-70, 70, 11)]
