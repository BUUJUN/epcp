...
cnn_res_JJ = cnn_res[(cnn_res.index.month>=6)&(cnn_res.index.month<=7)]
...
#%%
msl = read_dataarray('msl').loc[cnn_res_JJ.index, para.lat_circ, para.lon_circ]
z500 = read_dataarray('z', 500).loc[cnn_res_JJ.index, para.lat_circ, para.lon_circ]
u850 = read_dataarray('u', 850).loc[cnn_res_JJ.index, para.lat_circ, para.lon_circ][:, ::3, ::3]
v850 = read_dataarray('v', 850).loc[cnn_res_JJ.index, para.lat_circ, para.lon_circ][:, ::3, ::3]
u500 = read_dataarray('u', 500).loc[cnn_res_JJ.index, para.lat_circ, para.lon_circ][:, ::3, ::3]
v500 = read_dataarray('v', 500).loc[cnn_res_JJ.index, para.lat_circ, para.lon_circ][:, ::3, ::3]
...
#%%
epcp = cnn_res_JJ.index[(cnn_res_JJ.predict_ep==1)&(cnn_res_JJ.true_ep==0)]
extr = cnn_res_JJ.index[(cnn_res_JJ.predict_ep==1)&(cnn_res_JJ.true_ep==1)]
...
titles = ['Z500&uv500 nEPE-R JJ', 'SLP&uv850 nExtr JJ', 
          'Z500&uv500 EPE-R JJ', 'SLP&uv850 EPE-R JJ']
units  = ['$m^2 / s^2$', '$Pa$', '$m^2 / s^2$', '$Pa$']
cmaps  = ['anomaly', 'purple_orange', 'anomaly', 'purple_orange']
levels = [np.linspace(-250, 250, 11), 
          np.linspace(-120, 120, 9),
          np.linspace(-250, 250, 11),
          np.linspace(-120, 120, 9)]
