...
P_EPE_R_AM = PREC_ds_AM.prec.mean(dim=['lon', 'lat']).where(PREC_ds_AM.ep_day_sc==1)
RES_AM = partition(Patterns_AM, P_EPE_R_AM)
...
P_EPE_R_JJA = PREC_ds_JJA.prec.mean(dim=['lon', 'lat']).where(PREC_ds_JJA.ep_day_sc==1)
RES_JJA = partition(Patterns_JJA, P_EPE_R_JJA)
...
fig.savefig('./pics/FIG9_定量贡献分解_REPE.png')
fig.show()
