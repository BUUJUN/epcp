...
def sum_plot(axes, data_year, data_mon, ylabel_1, diff=True):
    ...
    axes[1].format(ylabel='Significance')
...
fig, axes = pplt.subplots(
    [[1, 2, 3],  
     [1, 2, 3],  
     [4, 5, 6]], figsize=(12, 5), hspace=(0))
axes.format(abcloc='ul')
sum_plot(axes[0:4:3], epcp_prec_yr_t, epcp_prec_ymon_t, 'Prec EPCP (mm/AMJJA)', diff=False)
sum_plot(axes[1:5:3], epcp_freq_yr_t, epcp_freq_ymon_t, 'Freq EPCP (days/AMJJA)', diff=False)
intensity_plot(axes[2:6:3], epcp_inte_yr_t, epcp_inte_ymon_t, 'Inte EPCP (mm/day)')
# figu.bar_plot_from_df(axes[11], contrib_df, colors=['blue7', 'red7', 'grey'])
axes[0].set_title('$corr_{obs} = $'+f'${calc.correlation(epcp_prec_yr_t, expr_yr_t)[0]:.2f}$', loc='right')
axes[1].set_title('$corr_{obs} = $'+f'${calc.correlation(epcp_freq_yr_t, exfr_yr_t)[0]:.2f}$', loc='right')
axes[2].set_title('$corr_{obs} = $'+f'${calc.correlation(epcp_inte_yr_t, exit_yr_t)[0]:.2f}$', loc='right')

axes[0].format(yticks=[-200, 0, 200])

import matplotlib.pyplot as plt
plt.savefig('./pics/FIG8_EPCP_PREC_AM.png')
plt.show()
#%%
