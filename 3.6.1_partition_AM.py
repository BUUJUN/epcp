...
# 筛选 4, 5
cnn_res = cnn_res[np.isin(cnn_res.index.month, [4, 5])]
ds = ds.sel(time=ds.time[np.isin(ds.time.dt.month, [4, 5])])
...
axes[4].format(title='Quantitative partitioning (AM)', titleloc='left', abcloc='l', 
               ylim=(-50, 50), yticks=[-40, -20, 0, 20, 40], 
               yticklabels=['-40', '-20', '0', '20', '40'], 
               ylabel='Changes (mm/AM)', )
axes[4].legend(ncol=2, loc='uc')
...
print(f'''
      极端降水变化: {delta_dt.values[0, 0]:.2f} mm/AM
...
# 单独画 (e)
importlib.reload(figu)
fig, axes = pplt.subplots([[1], [1], [0]], figsize=(6, 5), abcloc='ul', dpi=360)

figu.bar_plot_from_df(axes, delta_dt, colors=colors, width=1, legend=False)
axes.format(title='Quantitative partitioning (AM)', titleloc='left', abcloc='l', 
            ylim=(-20, 20), yticks=[-20, 0, 20], 
            yticklabels=['-20', '0', '20'], 
            ylabel='Changes (mm/AM)', )
axes.legend(ncol=2, loc='uc')

## ***********************
## * 相对贡献
## ***********************
aax = fig.add_axes(rect=[0.05, 0.06, 0.928, axes.get_position().height/2], )
aax.format(xticks=[], yticks=[])
aax.spines[:].set_visible(False)
...
units_abs = ['mm/AM', 'mm/AM']
units_rla = ['%', '%']
cmaps = ['anomaly', 'anomaly']
titles = ['Therm EPCP AM', 'Dynam EPCP AM']

levels_abs = [np.linspace(-60, 60, 11),
              np.linspace(-35, 35, 11)]
levels_rla = [np.linspace(-500, 500, 11),
