...
def partition(patterns, var, pls=None):
    import buujun.parameters as para
    import xarray as xr
    import numpy as np

    if pls is None: pls = set(patterns.data)
    
    nps = len(pls)
    ts = patterns.resample(time='Y').sum().time

    f_mean = np.empty((nps, ts.size))
    f_dist = np.empty((nps, ts.size))
    v_mean = np.empty((nps, ts.size))
    v_dist = np.empty((nps, ts.size))

    for i, p in enumerate(pls):
        freq_yr = (patterns==p).resample(time='Y').sum()

        inte_yr = freq_yr.copy()*np.nan
        var_pi_inte_yr = var.loc[patterns.time[patterns==p]].resample(time='Y').sum()
        inte_yr.loc[var_pi_inte_yr.time] = var_pi_inte_yr
        inte_yr = inte_yr / freq_yr
        
        f_mean[i] = freq_yr.mean()
        f_dist[i] = freq_yr - f_mean[i]
        v_mean[i] = xr.where(np.isnan(inte_yr), np.nan, 
                             inte_yr.mean(dim='time'))
        v_dist[i] = inte_yr - v_mean[i]
    
    f_dist = np.where(np.isnan(f_dist), 0, f_dist)

    dims = ['pattern', 'time']
    coords = dict(pattern=list(pls), time=ts)
    ...
    res_ds = xr.Dataset(data_vars=dict(total=total, dynam=dynam, therm=therm, nolin=nolin))
    return res_ds
...
# P_EPE_G_AM = PREC_ds_AM.prec.where(PREC_ds_AM.ep_day==1)
# RES_AM = partition(Patterns_AM, P_EPE_G_AM)
...
# P_EPE_G_JJA = PREC_ds_JJA.prec.where(PREC_ds_JJA.ep_day==1)
# RES_JJA = partition(Patterns_JJA, P_EPE_G_JJA)
...
#            ylim=(-50, 50), yticks=[-40, -20, 0, 20, 40], 
#            ylabel='Changes (mm/AM)', )
# ax1.legend(ncol=1, loc='uc', labelspacing=0.25)

# ax2 = axes[1]
# figu.bar_plot_from_df(ax2, Change_JJA_dt, **kwards_bar)
# ax2.format(title='Partitioning (JJA)', 
#            ylim=(-10, 90), yticks=[0, 20, 40, 60, 80], 
...
# fig.savefig('./pics/FIG9_定量贡献分解.png')
# fig.show()
