...
titles = ['Diff q&uv850', 'Diff w500&uv500', ]
units  = ['', '']
...
for i, ax in enumerate(axes[0:2]):
    ...
    cf = figu.diff_plot(ax, lon.data, lat.data, 
                        circ_cli[i].loc[para.PE].data, 
                        circ_cli[i].loc[para.PL].data, 
                        axis=0, 
                        cmap=cmaps[i], 
                        levels=levels[i],
                        extend='both', )
...
format_plot(axes[0], title='Diff w500 Clim')
cf = figu.diff_plot(axes[0], lon.data, lat.data, 
                    circ_cli[1].loc[para.PE].data, 
                    circ_cli[1].loc[para.PL].data, axis=0, 
...
format_plot(axes[1], title='Diff q EPCPs')
cf = figu.diff_plot(axes[1], lon.data, lat.data, 
                    circ_sel[0].loc[para.PE].data, 
                    circ_sel[0].loc[para.PL].data, axis=0, 
                    cmap=cmaps[0], levels=np.linspace(-24, 24, 13), extend='both', )
