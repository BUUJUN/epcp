...
titles = ['q&uv850 EPCP', 'divmf&uvmf EPCP', 'q&uv850 EPE-R', 'divmf&uvmf EPE-R']
units  = ['', '', '', '']
...
for i, ax in enumerate(axes):
    ...
    figu.anomaly_wind_plot(
        ax, lonu, latu, wind_comp_u[i], wind_comp_v[i], wind_clim_u[i], wind_clim_v[i], scale=scales[i])
    if i >= 2:
        ax.colorbar(cf, loc='right')

plt.savefig('./pics/FIG6_EPCP_水汽.png')
plt.show()
#%%