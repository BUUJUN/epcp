...
def read_dataarray(var_name, level=None):
    ...
    circ_array['time'] = pd.read_csv(para.model_result, index_col='time', 
                                     parse_dates=['time'], 
                                     date_parser=calc.date_parse).index
    return circ_array
...
lat = ds.lat.data
# 只保留极端降水部分的降水
prec_ep = xr.where(ds.ep_day_sc==1, ds.prec, np.nan)

q = read_dataarray('q').loc[:, para.lat_circ, para.lon_circ]
w = read_dataarray('w', level=500).loc[:, para.lat_circ, para.lon_circ]
#%%
prec_ep_epcp = ds.prec.loc[(ds.ep_day_sc==0)&(cnn_res.predict_ep==1).values]
q_ep_epcp = q.loc[(ds.ep_day_sc==0)&(cnn_res.predict_ep==1).values]
w_ep_epcp = w.loc[(ds.ep_day_sc==0)&(cnn_res.predict_ep==1).values]
# prec_ep_epcp = ds.prec.loc[(cnn_res.true_ep==1).values]
# q_ep_epcp = q.loc[(cnn_res.true_ep==1).values]
# w_ep_epcp = w.loc[(cnn_res.true_ep==1).values]
...
format_plot(axes[0], title='omega')
cf = figu2.contourf_plot(axes[0], w_ep_epcp.longitude, w_ep_epcp.latitude,
                    w_ep_epcp.mean(dim='time').data,
                    levels=np.linspace(-0.2, 0.2, 11),
                    extend='both', )
# cf = figu2.diff_plot(axes[0], prec_ep_epcp.lon, prec_ep_epcp.lat,
#                     prec_ep_epcp.loc[para.PE].data, 
#                     prec_ep_epcp.loc[para.PL].data, 
#                     extend='both', )
axes[0].colorbar(cf, loc='right')

format_plot(axes[1], title='Diff q')
cf = figu2.diff_plot(axes[1], q_ep_epcp.longitude, q_ep_epcp.latitude,
                    q_ep_epcp.loc[para.PE].data, 
                    q_ep_epcp.loc[para.PL].data,
                    cmap='anomaly',
                    levels=np.linspace(-50, 50, 11), 
                    extend='both', )
axes[1].colorbar(cf, loc='right')

format_plot(axes[2], title='Diff omega')
cf = figu2.diff_plot(axes[2], w_ep_epcp.longitude, w_ep_epcp.latitude,
                    w_ep_epcp.loc[para.PE].data, 
                    w_ep_epcp.loc[para.PL].data, 
                    cmap='anomaly',
                    levels=np.linspace(-0.05, 0.05, 11),
                    extend='both', )
axes[2].colorbar(cf, loc='right')
...
for i in [0, 1]:
    ...
    p = para.fill_mask(prec_ep.loc[pattern, :, :].resample(time='Y').sum())\
...
axes[4].format(title='Quantitative partitioning', titleloc='left', abcloc='l', 
               ylim=(-6, 45), yticks=[0, 20, 40, 60], yticklabels=['0', '20', '40', '60'], 
