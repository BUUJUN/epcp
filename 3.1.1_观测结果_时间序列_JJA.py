...
ds = ds.sel(time=np.isin(ds.time.dt.month, [6, 7, 8])) # 筛选四五月
...
sum_plot(axes[0:2], to_prec_yr, 'Total prec. (mm/JJA)')
sum_plot(axes[2:4], ep_prec_yr, 'Total extr. prec. (mm/JJA)')
sum_plot(axes[4:6], ep_freq_yr, 'EPE-R frequency (days/JJA)')
...
axes.format(abcloc='ul', abc='a')
sum_plot(axes[0:2], to_prec_yr_sc, 'Total prec. (mm/JJA)')
sum_plot(axes[2:4], ep_prec_yr_sc, 'Total extr. prec. (mm/JJA)')
sum_plot(axes[4:6], ep_freq_yr_sc, 'EPE-R frequency (days/JJA)')
