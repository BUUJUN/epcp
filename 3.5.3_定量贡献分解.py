...
def partition(patterns, var):
    ...
    v_mean = np.empty((2, lat.size, lon.size, year.size))
    v_dist = np.empty((2, lat.size, lon.size, year.size))

    for i in [0, 1]:
        freq_yr = (patterns==i).resample('Y').sum()

        var_pi_inte_yr = para.fill_mask(var.loc[patterns.index[patterns==i]]\
                                        .resample(time='Y').sum()) \
                                            / xr.DataArray(freq_yr)
        
        var_pi_inte_yr_ravg = var_pi_inte_yr.mean(dim=['lon', 'lat'])
    ...
    dims = ['pattern', 'lat', 'lon', 'time']
    coords = dict(pattern=['NEPCP', 'EPCP'],
                  lon=lon, lat=lat, time=year)
    v_mean_array = xr.DataArray(data=v_mean, dims=dims, coords=coords)
    v_dist_array = v_mean_array.copy(data=v_dist)
    f_mean_array = v_mean_array[:,0,0,:].copy(data=f_mean)
    f_dist_array = v_mean_array[:,0,0,:].copy(data=f_dist)
    ...
    res_dt = pd.DataFrame(
        data={'total':  total.mean(dim=['lon', 'lat']).sum(dim='pattern'), 
